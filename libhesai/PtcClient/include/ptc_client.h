...
{
public:
  PtcClient(std::string IP = kLidarIPAddr, uint16_t u16TcpPort = kTcpPort, bool bAutoReceive = false,
            PtcMode client_mode = PtcMode::tcp, uint8_t ptc_version = 1, const char* cert = nullptr,
            const char* private_key = nullptr, const char* ca = nullptr, uint32_t u32RecvTimeoutMs = 500,
  ...
  bool GetValFromOutput(uint8_t cmd, uint8_t retcode, const u8Array_t& payload, int start_pos, int length,
                        u8Array_t& res);
   ...
   *
   * @param speed                     Expected spinspeed, it is important to note that you must fill in the RPM
   * supported by the radar you are using
   * @return true
   * @return false
   */
