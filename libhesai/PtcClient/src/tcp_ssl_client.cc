...
bool TcpSslClient::Open(std::string IPAddr, uint16_t u16Port, bool bAutoReceive, const char* cert,
                        const char* private_key, const char* ca)
{
...
bool TcpSslClient::SetReceiveTimeout(uint32_t u32Timeout)
{
  ...
  int retVal = setsockopt(tcpsock_, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeout_ms, sizeof(timeout_ms));
#else
  ...
  int retVal = setsockopt(tcpsock_, SOL_SOCKET, SO_RCVTIMEO, (const void*)&timeout, sizeof(timeval));
#endif
  return retVal == 0;
}

int TcpSslClient::SetTimeout(uint32_t u32RecMillisecond, uint32_t u32SendMillisecond)
{
  if (tcpsock_ < 0)
  {
    printf("TcpClient not open\n");
  ...
  if (retVal == 0)
  {
    ...
    retVal = setsockopt(tcpsock_, SOL_SOCKET, SO_SNDTIMEO, (char*)&timeout_ms, sizeof(timeout_ms));
#else
    ...
    timeout.tv_sec = sec;
    timeout.tv_usec = msec * 1000;
    retVal = setsockopt(tcpsock_, SOL_SOCKET, SO_SNDTIMEO, (const void*)&timeout, sizeof(timeval));
#endif
  }
  return retVal;
}
