    ...
    if (0xee == ETheader.delimiter[0] && 0xff == ETheader.delimiter[1])
    {
      switch (ETheader.min_version)
      {
        case 1: {
          ...
          memcpy((void*)&m_ET_corrections.raw_azimuths, p, sizeof(int16_t) * channel_num);
          p += sizeof(int16_t) * channel_num;
          memcpy((void*)&m_ET_corrections.raw_elevations, p, sizeof(int16_t) * channel_num);
          ...
          printf("apha:%f, beta:%f, gamma:%f\n", m_ET_corrections.elevations[0], m_ET_corrections.elevations[1],
                 m_ET_corrections.elevations[2]);
...
int Udp2_4Parser<T_Point>::DecodePacket(LidarDecodedPacket<T_Point>& output, const UdpPacket& udpPacket)
{
  ...
  const HS_LIDAR_HEADER_ET_V4* pHeader =
      reinterpret_cast<const HS_LIDAR_HEADER_ET_V4*>(&(udpPacket.buffer[0]) + sizeof(HS_LIDAR_PRE_HEADER));
  // point to tails
  const HS_LIDAR_TAIL_ET_V4* pTail = reinterpret_cast<const HS_LIDAR_TAIL_ET_V4*>(
      &(udpPacket.buffer[0]) + pHeader->GetPacketSize() - sizeof(HS_LIDAR_TAIL_CRC_ET_V4) -
      sizeof(HS_LIDAR_TAIL_SEQ_NUM_ET_V4) - sizeof(HS_LIDAR_TAIL_ET_V4));
  ...
  //     );
  // }
  const HS_LIDAR_BODY_SEQ2_ET_V4* pSeq2 = reinterpret_cast<const HS_LIDAR_BODY_SEQ2_ET_V4*>(
      (const unsigned char*)pHeader + sizeof(HS_LIDAR_HEADER_ET_V4) + 2);
  const HS_LIDAR_BODY_LASTER_UNIT_ET_V4* pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V4*>(
      (const unsigned char*)pSeq2 + sizeof(HS_LIDAR_BODY_SEQ2_ET_V4));
  ...
  for (int blockId = 0; blockId < pHeader->GetBlockNum(); blockId++)
  {
    for (int seqId = 0; seqId < pHeader->GetSeqNum(); seqId++)
    {
      ...
      // pUnit next
      pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V4*>((const unsigned char*)pSeq2 +
                                                                       sizeof(HS_LIDAR_BODY_SEQ2_ET_V4));
    }
    if (blockId < (pHeader->GetBlockNum()))
    {
      // skip point id
      pSeq2 = reinterpret_cast<const HS_LIDAR_BODY_SEQ2_ET_V4*>((const unsigned char*)pSeq2 + 2);
      pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V4*>((const unsigned char*)pUnit + 2);
