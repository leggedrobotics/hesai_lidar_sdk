...
// The main purpose of this function is to open, read, and parse a lidar calibration file, and print appropriate
// messages based on the parsing results. This function can read both .bin and .csv files.
template <typename T_Point>
...
int Udp2_5Parser<T_Point>::LoadCorrectionCsvData(char* correction_string)
{
  ...
  while (std::getline(ifs, line))
  {
    ...
    {
      std::cout << "laser id is wrong in correction file. laser Id:" << laserId << ", line" << lineCount << std::endl;
      // return -1;
    }
    ...
    if (0xee == ETheader.delimiter[0] && 0xff == ETheader.delimiter[1])
    {
      switch (ETheader.min_version)
      {
        case 1: {
          ...
          memcpy((void*)&corrections_.raw_azimuths, p, sizeof(int16_t) * channel_num);
          p += sizeof(int16_t) * channel_num;
          memcpy((void*)&corrections_.raw_elevations, p, sizeof(int16_t) * channel_num);
          ...
          printf("apha:%f, beta:%f, gamma:%f\n", corrections_.elevations[0], corrections_.elevations[1],
                 corrections_.elevations[2]);
        ...
        case 2: {
          ...
          p = p + 1;
          int angle_offset_len = (120 / (corrections_.azimuth_adjust_interval * 0.5) + 1) *
                                 (25 / (corrections_.elevation_adjust_interval * 0.5) + 1);
...
int Udp2_5Parser<T_Point>::DecodePacket(LidarDecodedPacket<T_Point>& output, const UdpPacket& udpPacket)
{
  ...
  const HS_LIDAR_HEADER_ET_V5* pHeader =
      reinterpret_cast<const HS_LIDAR_HEADER_ET_V5*>(&(udpPacket.buffer[0]) + sizeof(HS_LIDAR_PRE_HEADER));

  // point to tails
  const HS_LIDAR_TAIL_ET_V5* pTail = reinterpret_cast<const HS_LIDAR_TAIL_ET_V5*>(
      &(udpPacket.buffer[0]) + pHeader->GetPacketSize() - sizeof(HS_LIDAR_CYBER_SECURITY_ET_V5) -
      sizeof(HS_LIDAR_TAIL_CRC_ET_V5) - sizeof(HS_LIDAR_TAIL_SEQ_NUM_ET_V5) - sizeof(HS_LIDAR_TAIL_ET_V5));
  ...
  //     );
  // }
  const HS_LIDAR_BODY_SEQ3_ET_V5* pSeq3 = reinterpret_cast<const HS_LIDAR_BODY_SEQ3_ET_V5*>(
      (const unsigned char*)pHeader + sizeof(HS_LIDAR_HEADER_ET_V5) + 2);
  const HS_LIDAR_BODY_LASTER_UNIT_ET_V5* pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V5*>(
      (const unsigned char*)pSeq3 + sizeof(HS_LIDAR_BODY_SEQ3_ET_V5));
  ...
  for (int blockId = 0; blockId < pHeader->GetBlockNum(); blockId++)
  {
    for (int seqId = 0; seqId < pHeader->GetSeqNum(); seqId++)
    {
      ...
      // pUnit next
      pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V5*>((const unsigned char*)pSeq3 +
                                                                       sizeof(HS_LIDAR_BODY_SEQ3_ET_V5));
    }
    if (blockId < (pHeader->GetBlockNum()))
    {
      // skip point id
      pSeq3 = reinterpret_cast<const HS_LIDAR_BODY_SEQ3_ET_V5*>((const unsigned char*)pSeq3 + 2);
      pUnit = reinterpret_cast<const HS_LIDAR_BODY_LASTER_UNIT_ET_V5*>((const unsigned char*)pUnit + 2);
...
int Udp2_5Parser<T_Point>::ComputeXYZI(LidarDecodedFrame<T_Point>& frame, LidarDecodedPacket<T_Point>& packet)
{
  ...
  for (int blockId = 0; blockId < packet.block_num; blockId++)
  {
    ...
    for (int i = 0; i < lasernum; i++)
    {
      ...
      float elv_v = raw_elevation * M_PI / 180 + theta * M_PI / 180 -
                    std::tan(raw_elevation * M_PI / 180) * (1 - std::cos(an * M_PI / 180));
      float delt_azi_v = std::sin(an * M_PI / 180) * std::cos(an * M_PI / 180) * theta_n * theta_n / 2 * 1.016 * M_PI /
                         180 * M_PI / 180;
      float eta = phi + delt_azi_v * 180 / M_PI + beta + raw_azimuth / 2;
      float delt_azi_h = std::sin(eta * M_PI / 180) * std::tan(2 * gamma * M_PI / 180) * std::tan(elv_v) +
                         std::sin(2 * eta * M_PI / 180) * gamma * gamma * M_PI / 180 * M_PI / 180;
