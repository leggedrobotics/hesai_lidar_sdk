...
struct ETCorrections
{
  ...
  float getAziAdjustV2(float azi, float ele) const
  {
    ...
    int offset_index1 =
        int((azi + azimuth_fov / 2) / (azimuth_adjust_interval * adjust_interval_resolution));  // azi dimension
    int offset_index2 =
        int((ele + elevation_fov / 2) / (elevation_adjust_interval * adjust_interval_resolution));  // ele dimension
    if (offset_index1 >= (azimuth_offset_num - 1) || offset_index2 >= (elevation_offset_num - 1))
      return 0;
    if (offset_index1 < 0 || offset_index2 < 0)
      return 0;
    float coefficient1 =
        ((offset_index1 + 1) * (azimuth_adjust_interval * adjust_interval_resolution) - azi - azimuth_fov / 2) /
        (azimuth_adjust_interval * adjust_interval_resolution);
    float coefficient2 =
        ((offset_index2 + 1) * (elevation_adjust_interval * adjust_interval_resolution) - ele - elevation_fov / 2) /
        (elevation_adjust_interval * adjust_interval_resolution);
    float offset1 = coefficient1 * azimuth_adjust[offset_index1 + offset_index2 * azimuth_offset_num] +
                    (1 - coefficient1) * azimuth_adjust[offset_index1 + 1 + offset_index2 * azimuth_offset_num];
    float offset2 = coefficient1 * azimuth_adjust[offset_index1 + (offset_index2 + 1) * azimuth_offset_num] +
                    (1 - coefficient1) * azimuth_adjust[offset_index1 + 1 + (offset_index2 + 1) * azimuth_offset_num];
  ...
  float getEleAdjustV2(float azi, float ele) const
  {
    ...
    unsigned int elevation_offset_num =
        int(elevation_fov / (elevation_adjust_interval * adjust_interval_resolution) + 1);
    int offset_index1 =
        int((azi + azimuth_fov / 2) / (azimuth_adjust_interval * adjust_interval_resolution));  // azi dimension
    int offset_index2 =
        int((ele + elevation_fov / 2) / (elevation_adjust_interval * adjust_interval_resolution));  // ele dimension
    if (offset_index1 >= (int)(azimuth_offset_num - 1) || offset_index2 >= (int)(elevation_offset_num - 1))
      return 0;
    if (offset_index1 < 0 || offset_index2 < 0)
      return 0;
    float coefficient1 =
    ...
    float coefficient2 =
        ((offset_index2 + 1) * (elevation_adjust_interval * adjust_interval_resolution) - ele - elevation_fov / 2) /
        (elevation_adjust_interval * adjust_interval_resolution);
    float offset1 = coefficient1 * elevation_adjust[offset_index1 + offset_index2 * azimuth_offset_num] +
                    (1 - coefficient1) * elevation_adjust[offset_index1 + 1 + offset_index2 * azimuth_offset_num];
    float offset2 = coefficient1 * elevation_adjust[offset_index1 + (offset_index2 + 1) * azimuth_offset_num] +
                    (1 - coefficient1) * elevation_adjust[offset_index1 + 1 + (offset_index2 + 1) * azimuth_offset_num];
    return (coefficient2 * offset1 + (1 - coefficient2) * offset2) / header.angle_division;
  }
};
