...
struct HS_LIDAR_HEADER_ET_V5
{
  ...
  uint16_t GetPacketSize() const
  {
    return (sizeof(struct HS_LIDAR_PRE_HEADER) + sizeof(struct HS_LIDAR_HEADER_ET_V5) +
            (sizeof(uint16_t) +
             (sizeof(struct HS_LIDAR_BODY_LASTER_UNIT_ET_V5) * (m_u8LaserNum / m_u8SeqNum) + 5) * m_u8SeqNum) *
                m_u8BlockNum +
            sizeof(struct HS_LIDAR_BODY_CRC_ET_V5) + sizeof(struct HS_LIDAR_TAIL_ET_V5) +
            sizeof(struct HS_LIDAR_TAIL_SEQ_NUM_ET_V5) + sizeof(struct HS_LIDAR_TAIL_CRC_ET_V5) +
